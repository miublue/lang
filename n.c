...
static int _is_operator(char *buf, int sz) {
  int i; for (i = 0; i < LENGTH(OPERATORS); ++i) {
...
static int _is_keyword(char *buf, int sz) {
  int i; for (i = 0; i < LENGTH(KEYWORDS); ++i) {
...
static int _precop(int kind, int prec) {
  int i; for (i = 0; i < LENGTH(PRECEDENCES[prec]); ++i)
...
void gen_file(FILE *out) {
  ...
  EMIT(".data\n");
  int i; for (i = 0; i < nstrs; ++i) {
    token_t *tok = strs[i];
    EMIT(".L__string.%d: .asciz \"%.*s\"\n.align 8\n",
        i, tok->sz, tok->ptr);
  }
}
...
int main(int argc, const char **argv) {
  ...
  for (i = 1; i < argc; ++i) {
    ...
    else if (!strcmp(argv[i], "-s")) output_stdout = 1;
    else if (!strcmp(argv[i], "-o")) output_path = argv[++i];
